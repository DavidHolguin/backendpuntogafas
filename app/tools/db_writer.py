...
def persist_order_result(job: AIOrderJob, result: FinalOrderResult) -> str | None:
    ...
    # ── 1. Insert prescription (if available) ─────────────────
    if result.prescription and result.prescription.rx_data:
        try:
    ...
    try:
        ...
        if order_resp.data:
            ...
            logger.info("Inserted order %s (number: %s)", order_id, order_number)
    except Exception as exc:
    ...
    for idx, item in enumerate(result.items):
        try:
            ...
            # Link prescription to lens items
            if prescription_id and item.description and "lente" in (item.description or "").lower():
